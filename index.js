...
function extractTextInParentheses(str) {
  ...
  for (const value of result) arr.push(value[1]);
...
function countEmails(str) {
  const regexp = /\b[\w\.%+-]+@[A-z\d\.-]+\.[A-z]{2,}\b/g;
...
function checkRegexFlags(regex) {
  const flugsArr = regex.flags;
  if (flugsArr.includes("g") && flugsArr.includes("m")) {
    return true;
  } else return false;
...
function checkFlags(regex) {
  const usedFlugs = [];
  if (regex.ignoreCase) usedFlugs.push("ignoreCase");
  const source = regex.source;

  return [usedFlugs, source].flat();
...
function checkRegexMethods(regex) {
  const usedFlugs = [];
  if (regex.dotAll) usedFlugs.push("dotAll");
  if (regex.multiline) usedFlugs.push("multiline");
  if (regex.sticky) usedFlugs.push("sticky");
